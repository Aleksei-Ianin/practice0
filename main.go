...
func NewIaninAleksei(name string, email string, notifier Notifier) *IaninAleksei {
	return &IaninAleksei{
		name:     name,
		email:    email,
		notifier: notifier,
...
func (u IaninAleksei) Notify(message string) error {
	fmt.Printf("IaninAleksei %s (%s) received a notification: ", u.name, u.email)
...
func main() {
	...
	user1 := NewIaninAleksei("Alice", "alice@example.com", emailNoti)
	user2 := NewIaninAleksei("Bob", "bob@example.com", smsNoti)
